...
int printMatrix(std::vector<std::vector<fraction> > &m){
...
fraction exponential(fraction a, long long b){
    //returns a^b
    fraction result = a;
    if(b == 0){
        result.numerator = 1;
...
int main() {
    ...
    for(int i = 0; i<numPoints; ++i){
        ...
        for(int i = 1; i<=numPoints; ++i){
            row.push_back(exponential(fractionParser(x), numPoints-i));
        }
        row.push_back(fractionParser(y));
        matrix.push_back(row);
    }
    if(numPoints == 1){
        std::cout << "f(x) = ";
        printFraction(matrix[0][1]);
        std::cout << "\n";
        return 0;
    }
    if(numPoints == 0){
        std::cout << "What you want me to do you doofus\n";
        return 0;
    }
    std::cout << pointFunction(matrix) << "\n";
    return 0;
